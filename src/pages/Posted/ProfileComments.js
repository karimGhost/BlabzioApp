...
for (const match of matches) {
  ...
  const newTag = match.replace(/\/?>/, `${widthAttr}${heightAttr} class="resized-imaged">`);
...
firebase.initializeApp(firebaseConfig2, 'app15');
const app5 = firebase.app('app15');
...
const  username = props.username;
                                          ...
                                          // Rating constructon start  setAllData
...
if (cardmessage.trim() !== '') {
  try {
    const snapshot = await app5.database().ref("commentpost")
      .orderByChild("ip")
      .equalTo(ip)
      .once("value");
    if (snapshot.exists()) {
      await app5.database().ref(`commentpost/${id}`).update({
        id: id,
        ip: ip,
      ...
      const userRef = app5.database().ref(`commentpost/${id}`);
      userRef.set({
        id: id,
        ip: ip,
        message: cardmessage.trim(),
...
function testit(){
  ...
  async function handleDeletereply(id)  {
    ...
    try {
      const commentRef = app5.database().ref("Replies/replied");
  ...
  async function handleDelete(id)  {
    ...
    try {
      const commentRef = app5.database().ref("commentpost");
...
const handleDismiss = (id) => {
  ...
  setCardreply((editing) => {
    ...
    return rest;
                                                    ...
                                                    //  reply coment construction start
...
try {
  ...
  if (snapshot.exists()) {
    ...
    const Replies = app5.database().ref(`Replies/`);
    Replies.set({
       replied,
    });
  }
...
useEffect(() => {
  if (user) {
    const commentsRef = app5.database().ref(`Replies/`);
...
const handleToggle = () => {
  ...
  useEffect(() => {
    ...
    const ratingsRef = app5.database().ref('Reply');
  ...
  async function handleDislikeClick(id) {
    ...
    try {
      ...
      if (snapshot.exists()) {
        const ratingKey = Object.keys(snapshot.val())[0]; // assuming there's only one matching rating
        const rating = snapshot.val()[ratingKey];
        await app5.database().ref(`Reply/${ratingKey}`).update({
          disliked: !rating.disliked,
          liked: false,
        });
      } else {
        const userRef = app5.database().ref('Reply');
        await userRef.push({
          cardId,
          user: props.user,
          liked: false,
...
useEffect(() => {
if (user) {
  const commentsRef = app5.database().ref(`commentpost/`);
...
return (
  ...
  <section className={` zIndex-2 ${props.dark &&  "darken"}`} style={{height: "fit-content", width:'100%', marginLeft:"auto", marginRight: "auto" }}>
            <h1>OOOFOFFOFOFOFOFOOFOFO</h1>

{props.user === username && <>
                                                   {/* PAge splits  container */}
                                                   { isBlogActive && props.user === username  ||   props.Bio === username  ?
 <div> 
        
        <Toast ref={toast} />
{!props.shared && 
  <div className={`py-1 mx-auto px-0 m ${  props.dark ? "bgswhiteBlack": "bgswhite"}`} style={{ }}>
    <Row style={{background: "transparent"}} className="justify-content-center m-0 p-0">
      <div style={{width:"100%"}} md="12" lg="10" xl="10" className=" w-100 p-0 mx-0 px-0" >
      <Card className={ `"p-0  carde  m-auto"   ${ props.dark ? "bb" :  'pp'}`}  style={{  color: props.dark ? "silver" : "black", background: props.dark && "#141515"  , width:"100%", padding: "0"  }}>
          <Card.Body className=" p-0">
          <Card.Body>
      { props.user === username ?
          <SpeedDial showIcon="" outlined  buttonClassName={`${ isOpen ? 'pi pi-times' : 'pi pi-ellipsis-h' } p-button-warning bold border-non p-button-outlined    sm   p-mt-2 p-0 p-2 w-2rem h-2rem`} model={items}  visible={isOpen}
    onShow={handleToggle}
  ...
  <SpeedDial showIcon=""  outlined  buttonClassName= {`  ${ isOpen ? ' pi pi-times':  'pi pi-ellipsis-h' }   p-button-success bold border-non  w-2rem h-2rem p-button-outlined sm   p-mt-1   p-2  '`} model={List}  visible={isOpen}
    onShow={handleToggle}
    onHide={handleToggle} direction="down" style={{padding: "2px",paddingLeft:"4px", zIndex: "3",position: "absolute", top: "0", left: "0",   color: "black", fontWeight:"bold"}} />
        }
   <div className="card"> 
...
<Button label="TopRight" icon="pi pi-arrow-down-left" onClick={() => show('top-right')} className="p-button-warning" style={{ minWidth: '10rem' }} /> */}
            ...
            <Link to={`/profileComponents/Profiles?id=${props.userId}`}>  
            <Card.Img
              ...
              /></Link>
              <div>
              <Link to={ props.username === props.user ? `/profileComponents/Myprofile` :  `/profileComponents/Profiles?id=${props.userId}`}>  
               <h5  className="fw-bold   mb-1 ml-2" style={{color:  props.dark ? "silver" : "black"}}>{props.username }{props.userId}</h5></Link>
...
<div className="w-auto p-0 "  style={{color: props.dark ? "white" : "black", marginLeft: "15%", marginRight: "10%", padding:"0px", width:"200%"}} >
<div ref={editRef} className="ppp"   onClick={ () =>  edited(props.ids)}   id={props.ids}  style={{background: "transparent",  cursor:"pointer", overflow:"hidden"}} >
...
<div style={{width: "100%", background: props.dark &&  "#141515"} } >
           ...
           <Link to={  props.username === props.user ? `/profileComponents/Myprofile` :       `/profileComponents/${props.copied[0].userId}`}>     <Card.Img
                className="rounded-circle shadow-1-strong me-3"
              ...
              /> </Link>
              <div style={{display: ""}}>
              <Link to={ props.username === props.user ? `/profileComponents/Myprofile` :    `/profileComponents/${props.copied[0].userId}`}>  <h5  className="fw-bold text-dark  mb-1">{copied[0].user}</h5></Link>
...
<>
<div ref={editRef} onClick={() =>  edited(props.ids)}  className='mb-0 Edits '  id={props.ids} style={{  cursor:"pointer", overflow:"hidden",  boxShadow: "inset 0px -28px 17px 1px rgba(0,0,0,0.19)", webkitBoxShadow: "inset 0px -28px 17px 1px rgba(0,0,0,0.19)",
moZboxShadow: "inset 0px -28px 17px 1px rgba(0,0,0,0.19)"
}}>
<div className="w-auto p-0 ppp"  style={{  color: props.dark ? "white" : "black", marginLeft: "auto", marginRight: "auto", padding:"0px", width:"200%"}} >

            {/*ReactHtmlParser(htmlCopied) */}

</div> 
</div>
{ useref &&  <button  onClick={() => edited(props.ids)} style={{ background:"rgba(1,1,1,0.11)", border:"none "}}>show more...</button> }
...
<Link to={`/pagesprofileComponents/Profiles?id=${props.userId}`}> 
<Card.Img 
         ...
         <Row style={{width: "100%", justifyContent:" center"}} key={card.id}>
           <div>
            <div className=" d-flex flex-start" >
             <Link to={`/profileComponents/Profiles?id=${card.userId}`}>  
               <Card.Img
                 className="rounded-circle shadow-1-strong me-3 "
                 src={card.profile}
                 alt="card.user"
       ...
       /> </Link>
               ...
               <div className="flex-grow-1 flex-shrink-1">
                 <div>
                   ...
                   <Link to={`/profileComponents/Profiles?id=${card.userId}`}> <p className="mb-1">
...
card.username !== props.user && props.username !== props.user &&
    ...
    onHide={handleToggle}  direction="down" style={{ zIndex: "3",position: "absolute", top: "0", right: "-30px",  border: "none", color: "black", fontWeight:"bold"}} />
              ...
              <div style={{marginBottom: "-10px", margin: "auto", paddingBottom:"0" }}  >
...
<div className="col-sm-flex-column">
   ...
   (car.ip === card.id && 
              ...
              {card.isactive ? <div>
                <Link to={`/profileComponents/${car.userId}`}>  
                     <Card.Img
                   ...
                   <div className="flex-grow-1 flex-shrink-1">
                     <div>
                       ...
                       <Link to={`/profileComponents/${car.userId}`}>   <p style={{color: props.dark ? "ghostwhite" : "black" }}className="mb-1">
                          {car.username}{" "}
         ...
         <div style={{borderBottom:"0.40px solid gray", margin: "auto", width:"95%" ,padding: "0px"}}></div>
         </Row>
         ))}
       </Card.Body>
     </Card>
