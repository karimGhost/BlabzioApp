...
const firebaseConfig90900 = {
 ...
 export default function MySaved(props){
...
const [editing, setEditing] = useState(false)
  ...
  if (index !== -1) {
    saved.splice(index, 1); // Remove the item from the current position
  }

  saved.push(item); // Add the item at the end of the array

  setSaved([...saved]); // Update the state with the modified array news
    }
...
<> </>
            ...
            {currentNews &&
              currentNews.slice(0, 10).map((newss, index) => (
                <div key={index} className="col-sm-6 col-lg-4 grid-margin">
